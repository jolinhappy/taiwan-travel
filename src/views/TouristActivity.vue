...
export default defineComponent({
  ...
  setup() {
    ...
    const getCityActivities = async(city: string, subCategory: string) => {
      try {
        if (!city) {
          const res = await apiHandler.getAllActivities();
          if (res) {
    ...
    const handleSearch = async(v: any) => {
      ...
      await getCityActivities(location as string, subCategory as string);
      await getTotalPages();
  ...
  async created() {
    ...
    await this.getCityActivities(location as string, subCategory as string);
